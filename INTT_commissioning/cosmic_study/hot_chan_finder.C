...
{   
    ...
    TH1F * nHit_before = new TH1F("nHit_before","nHit_before",200,0,200); // note : # of hit in each event before clone hit removal
    nHit_before -> SetLineColor(4);
    nHit_before -> GetXaxis() -> SetTitle("N hits");
    nHit_before -> GetYaxis() -> SetTitle("Entry");

    TH1F * nHit_after  = new TH1F("nHit_after","nHit_after",200,0,200); // note : # of hits in each event after the clone hit removal
    ...
    TH1F * NchHit_after  = new TH1F("NchHit_after","NchHit_after",300,0,1500); // note : After hit removal, # of hit each channel has in the whole file
